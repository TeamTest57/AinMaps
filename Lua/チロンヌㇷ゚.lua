local export = {}

export.gloss = "fox"

export.syns = {
	-- Hokkaido: Southern/Central
	["Abuta"]	= { "" },
	["Azuma"]	= { "" },
	["Biratori"]	= { "" },
	["Chitose"]	= { "" },
	["Ebeotsu"]	= { "" },
	["Hagino"]	= { "" },
	["Hidaka"]	= { "チロンヌㇷ゚:cironnup", "サㇰキモッペ:sakkimotpe((C2))" },
	["Hobetsu"]	= { "" },
	["Horobetsu"]	= { "チロンヌㇷ゚:cirónnup((H))", "パイカㇻマトゥㇺペ:paykarmatumpe((C2))" },
	["Horoman"]	= { "" },
	["Mitsuishi"]	= { "" },
	["Mukawa"]	= { "" },
	["Muroran"]	= { "" },
	["Niikappu"]	= { "" },
	["Nukkibetsu"]	= { "" },
	["Ogifushi"]	= { "" },
	["Oshamambe"]	= { "" },
	["Rebun"]	= { "フレㇷ゚:hurep", "フレㇷ゚チロンヌㇷ゚:hurepcironnup((C2))" },
	["Samani"]	= { "" },
	["Saru"]	= { "チロンヌㇷ゚:cirónnup", "スマリ:sumári(“おとなしいことば”)", "キモッペカムイ:kimótpekamuy((H))" },
	["Shiraoi"]	= { "" },
	["Shizunai"]	= { "" },
	["Takae"]	= { "" },
	["Takashima"]	= { "" },
	["Toyoura"]	= { "" },
	["Urakawa"]	= { "" },
	["Usu"]	= { "" },
	["Yakumo"]	= { "フレㇷ゚:húrep", "パクサウカムイ:pakusáw kamúy《神と人間の話を仲だちするもの》", "スマリ:長万部以北ではsumári と言う((H))" },

	-- Hokkaido: Northern
	["Asahikawa"]	= { "チロンノㇷ゚:cirónnop((H))" },
	["Chikabumi"]	= { "スマリ:sumari((C2))" },
	["Nayoro"]	= { "スマリ:sumári", "フレチロンヌㇷ゚:húrecironnup", "スマリチロンヌㇷ゚:sumáricironnup((H))" },
	["Soya"]	= { "スマリ:sumári((H))" },
	["Teshio"]	= { "スマリ:sumari((C2))" },

	-- Hokkaido: Eastern
	["Abashiri"]	= { "" },
	["Akan"]	= { "" },
	["Ashoro"]	= { "" },
	["Bihoro"]	= { "チロンノㇷ゚:cironnop((H))" },
	["Biroo"]	= { "" },
	["Chirotto"]	= { "" },
	["Fupushinai"]	= { "" },
	["Fushiko"]	= { "" },
	["Harutori"]	= { "" },
	["Kitami"]	= { "" },
	["Kushiro"]	= { "チロンヌㇷ゚:cironnup((C2))" },
	["Kussharo"]	= { "キモッペ:kimotpe((C2))" },
	["Memuro"]	= { "" },
	["Mokoto"]	= { "" },
	["Nemuro"]	= { "" },
	["Nijibetsu"]	= { "" },
	["Obihiro"]	= { "チロンノㇷ゚:cirónnop((H))" },
	["Otofuke"]	= { "" },
	["Shari"]	= { "" },
	["Shiranuka"]	= { "チロンヌㇷ゚:cironnup((C2))" },
	["Tokachi"]	= { "チロンヌㇷ゚:cironnup((C2))" },
	["Tooro"]	= { "" },

	-- Sakhalin: West Coast
	["Chirai"]	= { "" },
	["Maoka"]	= { "" },
	["Raichishka"]	= { "スマリ:sumari", "フーレカムイ:huurekamuy", "カウカウセㇸ:kawkawseh, -pihi", "レウケレウケㇸ:rewkerewkeh, -pihi((H))" },
	["Shiranushi"]	= { "" },
	["Tarantomari"]	= { "シトゥㇺペ:situmpe((C2))" },
	["Usoro"]	= { "" },

	-- Sakhalin: East Coast
	["Aihama"]	= { "" },
	["Nairo"]	= { "" },
	["Niitoi"]	= { "" },
	["Ochiho"]	= { "" },
	["Shirahama"]	= { "" },
	["Shiraura"]	= { "" },
	["Taraika"]	= { "" },
	["Tonnai"]	= { "" },
	["Yamambetsu"]	= { "" },

	-- Kuril: Northern
	["Kuril"]	= { "" },
	["Paramushir"]	= { "" },
	["Shumshu"]	= { "チャウチャウ:cawcaw?(キツネ)", "チャチョー:cacoo(キツネ)((V))" },

	-- Kuril: Southern
	["Etorofu"]	= { "" },
	["South Kuril"]	= { "チロンヌㇷ゚:cironnup(キツネ)", "フレレ:hure red(アカギツネ)", "シトゥㇺペ:situmpe(シルバー)((BS))" },
}

return export